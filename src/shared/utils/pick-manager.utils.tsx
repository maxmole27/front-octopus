...
import { BET_STATUS_HALF_LOSS, BET_STATUS_HALF_WIN, BET_STATUS_LOSE, BET_STATUS_PENDING, BET_STATUS_VOID, BET_STATUS_WON } from '@/ui/constants'
...
export function handleStatusParlay (betslip: BetslipResponse) {
  ...
  if (statusList.includes('WON') && statusList.includes('VOID') && !statusList.includes('LOST')) return 'PARTIAL WIN'
  if (statusList.includes('VOID') && !statusList.includes('LOST') && !statusList.includes('WON')) return 'VOID'
}
...
export function getRealOddsParlay (betslip: BetslipResponse) {
  return betslip.individual_bets.reduce((acc, bet) => acc * bet.odds, 1).toFixed(2)
}
...
export function getAllParlayPicks (betslip: BetslipResponse): React.ReactNode {
  return (<>
    ...
    {betslip.individual_bets.map((bet: IndividualBet, index: number) => {
      return (
        <div key={index}>
          <span>{bet.player_or_team1.name} vs {bet.player_or_team2.name}</span>
          <br />
        </div>
      )
    })}
